      ...
      {movers.map((stock) => {
        ...
        return (
          <motion.div 
            ...
            className="group bg-card/40 backdrop-blur-xl border border-border/50 rounded-2xl p-5 cursor-pointer hover:border-accent/50 hover:bg-card/60 transition-all duration-300 shadow-[0_4px_24px_rgba(0,0,0,0.2)] hover:shadow-[0_8px_32px_rgba(0,229,255,0.15)] flex flex-col relative overflow-hidden"
          >
            ...
            <div className="flex justify-between items-start mb-3 relative z-10">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-xl bg-muted/80 border border-border flex items-center justify-center text-[11px] font-bold text-foreground shadow-inner group-hover:border-accent/30 transition-colors">
                  {stock.symbol[0]}
                </div>
                <div>
                  <h3 className="font-bold text-foreground tracking-tight">{stock.symbol}</h3>
                  <p className="text-[10px] text-muted-foreground truncate max-w-[100px] font-medium">{stock.name}</p>
                </div>
              </div>
              <div className={`p-1.5 rounded-lg ${isBullish ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger'}`}>
                <TrendingUp className="w-4 h-4" />
              </div>
            </div>
            
            <div className="flex items-baseline gap-2 mt-1 relative z-10">
              <span className="text-2xl font-bold text-foreground tracking-tight drop-shadow-sm">${stock.price.toFixed(2)}</span>
              <span className={`text-xs font-bold flex items-center ${isBullish ? 'text-success drop-shadow-[0_0_5px_rgba(0,230,118,0.5)]' : 'text-danger drop-shadow-[0_0_5px_rgba(255,23,68,0.5)]'}`}>
